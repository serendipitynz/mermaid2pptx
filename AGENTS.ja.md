...
判定は末尾の `VERDICT:` 行を読む。ジェネレータが `stCxn`/`endCxn` を出さなくなると
動かしたノードに何も接続されていない状態になり、数値だけ見ると `followed=0 stuck=0` で
正常な実行と見分けがつかない。未接続の `edge ` 端点が 1 つでもあれば、動かしたノード
自身の端点がすべて追従していても FAIL とする。未接続端点はシェイプ名を持たないため、
失われたバインドが動かしたノードのものだったか判別できないからである。ジェネレータが
出す `edge ` コネクタは常に両端がバインドされているので、これで正当な deck を弾くことはない。

既知の死角: U ターン経路はフリーフォームのポリラインにフォールバックし、コネクタでは
なく `<p:sp>` として出力される (`writeFreeformEdge`)。PowerPoint はこれをコネクタとして
報告しないため、このスクリプトからは見えない。`OK` 判定はそれらのエッジについては
何も述べていない。
...
- **出力フォントを測る仕組みはパイプライン内に存在しない**(標準ライブラリのみの
  ためフォントメトリクスを持てない)。幅は Mermaid がブラウザ自身のフォントスタックで
  測った実測値に由来する。例外は `sequence.go` で、読み取れる実測ボックスが SVG に
  無いため `estTextWidth` が文字種ごとに推定する。`slide.go` の `labelWidthSafety`
  は既定フォントぶんの幅の差を吸収するもので、それより大幅に幅の広いフォント(等幅
  など)を `-font` に指定すると、エッジラベルに限らずノードラベルでも箱からはみ出す。
  任意のフォントを賄える値まで広げると既定出力が恒常的に緩くなるため、既定フォントに
  合わせてある。
