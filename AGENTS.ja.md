...
- **テキストの寸法はすべて Mermaid のブラウザ実測値に由来する。** 測定に使われる
  のはブラウザ自身のフォントスタックで、出力フォントを測る仕組みはパイプライン内に
  存在しない(標準ライブラリのみのためフォントメトリクスを持てない)。`slide.go` の
  `labelWidthSafety` は既定フォントぶんの幅の差を吸収するもので、それより大幅に
  幅の広いフォント(等幅など)を `-font` に指定すると、エッジラベルに限らず
  ノードラベルでも箱からはみ出す。任意のフォントを賄える値まで広げると既定出力が
  恒常的に緩くなるため、既定フォントに合わせてある。
